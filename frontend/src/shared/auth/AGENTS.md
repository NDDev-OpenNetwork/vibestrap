...
- Keep cache keys session-specific and preserve AuthCacheBoundary's logout/session-switch cleanup.
- Admin plugin operations have a separate explicit allowlist in admin-access.ts, checked on its server.
- Regenerate types with `bun run api:generate`; run `bun run test:auth`, typecheck and FSD checks.
